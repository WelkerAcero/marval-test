    ...
    static async validateInputs(data: any, validation?: any): Promise<VALIDATION_TYPE> {
        ...
        if (typeof validation == 'object') {
            for (const key in data) {
                ...
                if (key == 'nit') if (!this.onlyNumbers(data[key], RULE)) return { error: this.message(`"Nit" debe estar entre ${RULE.min} a ${RULE.max} números`), valid: false };
                ...
                if (key == 'provider_type') if (!this.letterString(data[key], RULE)) return { error: this.message('"Tipo de proveedor" no debe contener caracteres inválidos como simbolos u otros'), valid: false };
                if (key == 'person_type') if (!this.letterString(data[key], RULE)) return { error: this.message(`"Tipo de persona" no debe contener caracteres inválidos como simbolos u otros`), valid: false };
                if (key == 'status') if (!this.onlyNumbers(data[key], RULE)) return { error: this.message(`"Estado" no debe contener caracteres inválidos como simbolos u otros`), valid: false };
            }
        }
